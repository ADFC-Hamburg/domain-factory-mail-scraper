...
def login(driver: WebDriver, username:str, password:str):
    ...
    login_button = driver.find_element(By.XPATH, '//*[@id="root"]/div[2]/div/div[1]/main/div/div/div/div/div/div[1]/form/span/button')
    login_button.click()
...
def analyse_email_mainpage(driver: WebDriver):
    ...
    user=[]
    for row in rows:
        addr=row.find_element(By.XPATH,'./td[1]/table/tbody/tr/td[2]').get_attribute('innerHTML').strip()
        alt_addr=None
        if addr.startswith('<span'):
            addr=row.find_element(By.XPATH,'./td[1]/table/tbody/tr/td[2]/span').get_attribute('data-title').strip()
        if '<small>' in addr:
            alt_addr=addr.split('<small>')[1].replace('</small>','')
            addr=addr.split('<br>')[0]
        size_str=row.find_element(By.XPATH,'./td[11]').get_attribute('innerHTML').strip().replace(' MB','')
        if size_str=='' or size_str==MINUS_CHAR:
            mb_size=None;
        else:
            mb_size=int(size_str)
        autoresponder=(row.find_element(By.XPATH,'./td[2]/img').get_attribute('alt')==OK_CHAR)
        forwarder=(row.find_element(By.XPATH,'./td[6]/img').get_attribute('alt')==OK_CHAR)
        destinations=[]
        if (forwarder):
            row12=row.find_element(By.XPATH,'./td[12]').get_attribute('innerHTML').strip()

            if row12.startswith('<span'):
                dest_txt=row.find_element(By.XPATH,'./td[12]/span').get_attribute('data-title').split('<br>')
...
def reset_password(driver: WebDriver, url, new_password:str):
    ...
    driver.find_element(By.XPATH,'/html/body/section/article/section/div[11]/form/table/tbody/tr/td[1]/button').click()
    sleep(1)
def main():
